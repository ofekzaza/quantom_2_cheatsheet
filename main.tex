...
\usepackage{multicol}
\usepackage[left=0.5cm, right=0.5cm, top=1.3cm, bottom=0.5cm, a4paper]{geometry}
\usepackage{graphicx}
\usepackage{wrapfig}
\usepackage{enumitem}
\usepackage{physics}
...
\setlength{\headsep}{0.0001cm} % Space between header and text
...
\textbf{Definitions:} $Z$ = Atomic number (Nucleus charge $+Ze$).
$L_q^p(x)$ = Assoc. Laguerre Polys: $L_q^p(x) = \frac{x^{-p} e^x}{q!} \frac{d^q}{dx^q}(e^{-x} x^{p+q})$.
...
$[a, a^\dagger] = 1, \quad \mathcal{H} = \hbar\omega(a^\dagger a + 1/2)$.
...
\important{\frac{d}{dt} O_H = \frac{i}{\hbar} [\mathcal{H}, O_H] + \left(\frac{\partial O}{\partial t}\right)_H}
...
\textbf{Diamagnetic:} $\mathcal{H}_D = \frac{e^2}{8m} (\mathbf{r} \times \mathbf{B})^2$. Important if $B$ large or $n$ large.
