...
Hamiltonian separates: $H = H_{CM} + H_{rel}$.
\important{H_{CM} = \frac{\mathbf{P}^2}{2M}, \quad H_{rel} = \frac{\mathbf{p}^2}{2\mu} + V(r)}
$\Psi(\mathbf{R}, \mathbf{r}) = e^{i\mathbf{K}\cdot\mathbf{R}} \psi(\mathbf{r})$. $E_{tot} = E_{CM} + E_{rel}$.
$H_{rel}$ is effectively a 1-body problem with mass $\mu$.
...
\section*{Mathematical Preliminaries (T1)}
...
1. \textbf{Anti-linear}. 2. Preserves probability: $|\langle A\phi | A\psi \rangle| = |\langle \phi | \psi \rangle|$.
...
$[L^2, L_i] = 0, \quad [L^2, \mathbf{x}] = 0$ (Scalar op? No, vector).
Actually $[L^2, V_q] \neq 0$. But $[L_i, r^2] = 0, [L_i, p^2] = 0$.
\textbf{BCH Formula:} $e^A e^B = e^{A+B+\frac{1}{2}[A,B] + \dots}$
Hadamard: $e^A B e^{-A} = B + [A,B] + \frac{1}{2!}[A,[A,B]] + \dots$
...
Anti-commutator: $\{\sigma_i, \sigma_j\} = 2\delta_{ij} I$.
...
$a = \sqrt{\frac{m\omega}{2\hbar}} (x + \frac{i}{m\omega}p), \quad a^\dagger = \sqrt{\frac{m\omega}{2\hbar}} (x - \frac{i}{m\omega}p)$.
\important{x = \sqrt{\frac{\hbar}{2m\omega}}(a + a^\dagger), \quad p = i\sqrt{\frac{m\hbar\omega}{2}}(a^\dagger - a)}
$[a, a^\dagger] = 1, \quad H = \hbar\omega(a^\dagger a + 1/2)$.



\section*{Tirgul Summaries (T2-T3)}

\subsection*{Symmetry \& Conservation (T2)}
\textbf{Noether's Theorem:} Continuous symmetry $\implies$ conservation.
- Time Translation ($t \to t+\epsilon$) $\implies$ Energy ($H$) cons.
...
\textbf{Parity ($\pi$ or $P$):} Unitary, $\pi^\dagger = \pi^{-1} = \pi$.
...
\subsection*{Angular Momentum (T3)}
\textbf{Commutators:}
\important{[L_i, L_j] = i\hbar \epsilon_{ijk} L_k, \quad [L^2, L_i] = 0}
Eigenvalues: $L^2 |l,m\rangle = \hbar^2 l(l+1) |l,m\rangle, \; L_z |l,m\rangle = \hbar m |l,m\rangle$.

\textbf{Spherical Harmonics $Y_l^m(\theta,\phi)$:}
Defined by separating $\nabla^2$ in spherical coords.
$Y_l^m(\theta,\phi) \propto P_l^m(\cos\theta) e^{im\phi}$ ($P_l^m$ Assoc. Legendre).
\important{Y_l^m(\theta,\phi)^* = (-1)^m Y_l^{-m}(\theta,\phi)}
Orthogonality: $\int d\Omega Y_{l'}^{m'*} Y_l^m = \delta_{ll'} \delta_{mm'}$.
Parity: $Y_l^m(\pi-\theta, \phi+\pi) = (-1)^l Y_l^m(\theta, \phi)$.

\subsection*{Heisenberg Picture (T4)}
States fixed, operators evolve: $O_H(t) = U^\dagger(t) O_S U(t)$.
\important{\frac{d}{dt} O_H = \frac{i}{\hbar} [H, O_H] + \left(\frac{\partial O}{\partial t}\right)_H}
Example: Free particle $\dot{x} = p/m, \dot{p} = 0$. $x(t) = x(0) + p(0)t/m$.

\subsection*{Spherical Potentials (T5)}
...
\subsection*{Atom in Magnetic Field (T6)}
$H = \frac{1}{2m}(\mathbf{p} + e\mathbf{A})^2 - e\phi$. Gauge $\mathbf{A} = \frac{1}{2}\mathbf{B} \times \mathbf{r}$.
$H \approx H_0 + H_P + H_D$.
\textbf{Paramagnetic (Zeeman):} $H_P = -\frac{e}{2m} \mathbf{L} \cdot \mathbf{B} = \mu_B \mathbf{L} \cdot \mathbf{B} / \hbar$.
\textbf{Diamagnetic:} $H_D = \frac{e^2}{8m} (\mathbf{r} \times \mathbf{B})^2$. Important if $B$ large or $n$ large.

\subsection*{Addition of Angular Momentum (T8)}
$J = J_1 + J_2$. Basis $|j_1 j_2; m_1 m_2\rangle \to |j_1 j_2; j m\rangle$.
Clebsch-Gordan Coeffs: $C_{m_1 m_2}^{j m} = \langle j_1 j_2; m_1 m_2 | j m \rangle$.
\important{|j_1 j_2; j m\rangle = \sum_{m_1, m_2} C_{m_1 m_2}^{j m} |j_1 j_2; m_1 m_2\rangle}
Selection: $m = m_1 + m_2, \quad |j_1 - j_2| \le j \le j_1 + j_2$.
Orthogonality: $\sum_{m_1 m_2} C_{m_1 m_2}^{j m} C_{m_1 m_2}^{j' m'} = \delta_{jj'} \delta_{mm'}$.

\subsection*{Tensor Operators (T9)}
Vector op $V_i$ commutes with $J$ like $r$ or $p$.
Generic Tensor $T_k^q$ transforms like $Y_k^q$ under rotation.
\important{[J_z, T_k^q] = \hbar q T_k^q}
\important{[J_\pm, T_k^q] = \hbar \sqrt{k(k+1)-q(q\pm 1)} T_k^{q\pm 1}}
...
Reduced Matrix Element $\langle || \rangle$ is independent of $m, m', q$.

\subsection*{Perturbation Theory (T10-T11)}
...
\textbf{Time-Dependent:} $H(t) = H_0 + V(t)$. $c_n(t) = \langle n | \psi(t) \rangle e^{iE_n t/\hbar}$.
\important{\dot{c}_n(t) = -\frac{i}{\hbar} \sum_k V_{nk}(t) e^{i\omega_{nk}t} c_k(t)}
First Order: $c_n^{(1)}(t) = -\frac{i}{\hbar} \int_{t_0}^t \langle n | V(t') | i \rangle e^{i\omega_{ni}t'} dt'$.
 Fermi's Golden Rule (Transition Rate $i \to f$):
\important{W_{i\to f} = \frac{2\pi}{\hbar} |V_{fi}|^2 \delta(E_f - E_i)}

\subsection*{Identical Particles (T12)}
\textbf{Symmetry:} Bosons (Sym), Fermions (Anti-Sym).
Helium Atom: 

$H = \frac{p_1^2}{2m} + \frac{p_2^2}{2m} - \frac{2e^2}{r_1} - \frac{2e^2}{r_2} + \frac{e^2}{r_{12}}$.
...
\subsection*{Scattering - Born Approx (T13)}
Lippmann-Schwinger: 
...
Valid if potential is weak or energy high.
