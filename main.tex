...
1. \textbf{Anti-linear}: $A(c_1|\psi\rangle + c_2|\phi\rangle) = c_1^* A|\psi\rangle + c_2^* A|\phi\rangle$.
2. Preserves propability (conjugate inner product):
\important{\langle A\phi | A\psi \rangle = \langle \phi | \psi \rangle^*}

\textbf{Anti-Hermitian:} $X^\dagger = -X$. (Implied).
...
\textbf{Noether's Theorem:} Symmetry of action $S \implies$ Conserved quantity.
...
\textbf{Rotation Group $SO(3)$:} $R^T R = I, \det R = 1$.
...
\important{[J_z, T_k^q] = \hbar q T_k^q, \quad [J_\pm, T_k^q] = \hbar \sqrt{k(k+1)-q(q\pm 1)} T_k^{q\pm 1}}
...
\textbf{Time-Independent (Non-Degenerate):} $H = H_0 + \lambda V$.
$E_n^{(1)} = V_{nn}$.
\important{|n^{(1)}\rangle = \sum_{k \neq n} \frac{V_{kn}}{E_n^{(0)} - E_k^{(0)}} |k^{(0)}\rangle, \quad E_n^{(2)} = \sum_{k \neq n} \frac{|V_{kn}|^2}{E_n^{(0)} - E_k^{(0)}}}
...
Helium Atom: $H = \frac{p_1^2}{2m} + \frac{p_2^2}{2m} - \frac{2e^2}{r_1} - \frac{2e^2}{r_2} + \frac{e^2}{r_{12}}$.
...
Born Series: $|\psi\rangle = |\phi\rangle + G_0^+ V |\phi\rangle + \dots$
